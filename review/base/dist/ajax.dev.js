...
var ajax = function ajax(option) {
  // .0 创建异步对象实例
  var xhr = new XMLHttpRequest(); // .0 监听请求和响应数据的状态
  ...
  xhr.onreadystatechange = function () {
    ...
    if (xhr.readyState === 4) {
      ...
      if (xhr.status == 200) {
        // .0 数据响应回来
        var resInfo = xhr.getResponseHeader("Content-Type"); // .0 接收响应报文的变量
  ...
  for (var key in option.data) {
    ...
    dataStr += "".concat(key, "=").concat(option.data[key], "&");
  } // .0 去掉最后一个字符 &
  ...
  xhr.open(option.type, option.type == "get" ? option.url + "?" + dataStr : option.url, true); // .0 判断请求方式
